...
║  ✅ EasyOCR → ref number ከ screenshot ያወጣል                      ║
║  ✅ CBE: URL ላይ ያለ ref (FT...) ያወጣል                            ║
║  ✅ Telebirr: Transaction Number (DE...) ያወጣል                   ║
║  ✅ Ref miss ከሆነ → user manually ይጽፋል                          ║
║  ✅ SMS Forwarder → real bank SMS bot ላይ ይልካል                   ║
║  ✅ Ref match → SMS amount → Auto Approve                        ║
...
📦 Install:
    pip install pyTelegramBotAPI firebase-admin flask easyocr pillow requests
...
# ══════════════════════════════════════════════════════
#  🔍 EasyOCR — lazy load (Railway memory ለመቆጠብ)
# ══════════════════════════════════════════════════════
_ocr_reader = None
...
# ══════════════════════════════════════════════════════
#  📸 OCR — Screenshot ላይ ያለ Ref ማውጣት
# ══════════════════════════════════════════════════════
def extract_ref_from_screenshot(file_id: str) -> str | None:
    """
    Screenshot download → EasyOCR → ref extract
    CBE:      FT + alphanumeric (e.g. FT261241NS84)
    Telebirr: DE + alphanumeric (e.g. DE33I1U0W7)
    """
    try:
        ...
        # ── Preprocess: grayscale + resize for better OCR ──
        ...
        # ── Extract Ref ──
        ref = extract_ref_from_text(full_text)
        return ref
...
def extract_ref_from_text(text: str) -> str | None:
    ...
    CBE URL format:
      https://Mbreciept.cbe.com.et/FT261241NS84-41057146
      → ref = FT261241NS84

    Telebirr format:
      Transaction Number: DE33I1U0W7
      Your transaction number is DE33I1U0W7
      → ref = DE33I1U0W7
    """
    # ── CBE URL ref ──
    # URL ውስጥ: /FT261241NS84-41057146 ወይም /FT261241NS84
    cbe_url = re.search(
        r'(?:cbe\.com\.et[/\s]+|Mbreciept[^\s]*/\s*)(FT[A-Z0-9]{6,14})',
        text, re.IGNORECASE
    )
    if cbe_url:
    ...
    tel_ref = re.search(
        r'(?:transaction\s*number\s*(?:is)?\s*:?\s*)(DE[A-Z0-9]{6,12})',
        text, re.IGNORECASE
    )
    if tel_ref:
        return tel_ref.group(1).upper()

    # ── Telebirr ref standalone ──
...
def save_ref(ref_no: str, uid: str, amount: int):
    fb_set(f"bot/used_refs/{ref_no.upper()}", {
        "user_id": uid,
        "amount":  amount,
        "time":    datetime.now().isoformat()
    })
...
def parse_cbe_sms(text: str) -> dict | None:
    """
    CBE SMS formats:
    1. "received ETB 100.00 from account 1********0083"
    2. "has been credited with ETB 1"
    """
    ...
    # Amount
    amt_m = re.search(
        r'(?:received ETB|credited with ETB|Credited with ETB)\s+([\d,]+\.?\d*)',
        text, re.IGNORECASE
    )
    if not amt_m:
        return None
...
def parse_telebirr_sms(text: str) -> dict | None:
    ...
    1. "received ETB X from name(phone) on DD/MM/YYYY. Your transaction number is DE..."
    2. "received ETB X by transaction number DE... on YYYY-MM-DD from Bank"
    """
    text = re.sub(r'\s+', ' ', text.strip())
...
def extract_date(text: str) -> str | None:
    """DD/MM/YYYY ወይም YYYY-MM-DD format ያወጣል → DD/MM/YYYY ያደርጋል"""
    # DD/MM/YYYY
...
def process_screenshot(m):
    """Background thread — screenshot OCR + match"""
    ...
    if ref:
        print(f"✅ OCR ref extracted: {ref}")
        # Duplicate ref check
        if is_dup_ref(ref):
            bot.send_message(m.chat.id,
                f"🚫 <b>Ref <code>{ref}</code> አስቀድሞ ጥቅም ላይ ዋሏል!</b>\n"
                "Duplicate payment አይቀበልም።")
            fb_set(f"temp/{uid}", None)
            return

        # Save payment with ref
        ...
        # ── Check if matching SMS already exists ──
        ...
        # ── OCR ref ማውጣት አልቻለም → user manually ይጽፋል ──
        print(f"⚠️ OCR failed for {uid} — asking manual ref")
        fb_set(f"temp/{uid}/file_id",        file_id)
        fb_set(f"temp/{uid}/waiting_manual_ref", True)
        fb_set(f"bot/state/{uid}",           "waiting_ref")

        bot.send_message(m.chat.id,
            f"📸 Screenshot ተቀብሏል!\n\n"
            f"⚠️ Ref number ማውጣት አልቻለም\n\n"
            f"📋 <b>Transaction/Ref number ብቻ ጻፍ:</b>\n"
            f"• CBE: <code>FT261241NS84</code>\n"
            f"• Telebirr: <code>DE33I1U0W7</code>")


def try_match_sms_by_ref(ref: str) -> dict | None:
    """Ref ጋር matching pending SMS ያፈልጋል"""
    ...
    for sms_key, sms_data in sorted_sms:
        ...
        if sms_ref and sms_ref.upper() == ref.upper():
            return {
                "key":    sms_key,
                "bank":   sms_data.get("bank"),
                "amount": sms_data.get("amount"),
                "sender": sms_data.get("sender"),
                "ref":    sms_data.get("ref"),
            }
    return None
...
def handle_screenshot(m):
    # Background thread — OCR ጊዜ ይወስዳል
...
@bot.message_handler(
    func=lambda m: (
        ...
        m.text and
        ("credited with etb" in m.text.lower() or
         "has been credited" in m.text.lower() or
         "you have received etb" in m.text.lower() or
         "received etb" in m.text.lower())
    )
)
def handle_forwarded_sms(m):
    ...
    # ── Find matching pending payment by REF ──
    payments = fb_get("payments") or {}
    ...
    # ── No ref match → try amount match (FALLBACK) ──
    if not matched_pid:
        for pid, pay in sorted_payments:
    ...
    if state == "waiting_ref":
        ...
        if matched:
            ...
            bot.send_message(m.chat.id,
                ...
                f"⏳ SMS verification እየጠበቀ ነው...\n"
                f"ከ5 ደቂቃ ውስጥ SMS ካልደረሰ auto cancel ⚠️")
        return
...
def handle_callback(c):
    ...
    if data == "deposit":
        ...
        for p in user_txns[:10]:
            ...
            ref_str  = f" | Ref: {p.get('ref','—')}" if p.get("ref") else ""
            lines.append(f"{icon}{verified} {p.get('amount',0)} ብር — {t}{ref_str}")
        bot.send_message(c.message.chat.id, "\n".join(lines))
...
# ══════════════════════════════════════════════════════
print("🤖 Bingo Bot starting with EasyOCR + Auto-Restart...")
